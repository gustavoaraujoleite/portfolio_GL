...
export default function NavButton({ href, title, pathProp }: NavButtonProps) {
  ...
  const pathPicker = (pathProp ?? pathName ?? "") as string;
  const splittedPath = pathPicker?.split("/");
  const path = "/" + splittedPath[splittedPath?.length - 1];

  const splittedHref = href?.split("/");
  const formattedHref = "/" + splittedHref[splittedHref?.length - 1];

  const isActive = path === formattedHref;
